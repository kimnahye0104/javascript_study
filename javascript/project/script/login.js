...
//id, pw가 모두 입력 기준으로 id가 admin이 아니면 '아이디를 확인하세요' error출력
//id,pw가 모두 입력된 걸 기준으로 id가 admin이 맞고 pw가 1234가 아니면 '비밀번호를 확인하세요.' error 출력
...
//id, pw 입력하지 않고 로그인 클릭시 error 위치에 "정보를 입력하세요." 출력
//1. 클릭 이벤트 생성
//2. 조건문으로 id,pw 입력했는가
...
login_btn.addEventListener('click',() => {
    ...
    if(user_id.value == '' || user_pw.value == ''){
        //3. 위 2번이 입력안했다면 "정보를 입력하세요"
        //3-1. 아이디를 안적고 로그인을 눌렀다면, '아이디를 입력하세요.'
        //3-2. 비번을 안적고 로그인을 눌렀다면, '비번을 입력하세요'
        //3-3. 둘다 안 적었다면, '아이디와 비밀번호를 입력하세요.
        if(user_id.value=='' && user_pw.value==''){
        ...
        // (아래 자바스크립트 디자인 코드를 css에 불러와 편하게 쓰는법)
